let editor = {
    ...
    draw: function (struct = false, root = false) {
        ...
        for (i in struct) {
            ...
            if (item.id) {
                if (self.canUpdate) {
                    let contentActionEdit = $('<a/>').attr({ class: 'menu-action text-dark', href: 'javascript:void(0);', target: '_self' }).append($('<i/>').attr({ class: 'fa fa-edit' }));
                ...
                if (self.canDelete) {
                    let contentActionDelete = $('<a/>').attr({ class: 'menu-action text-danger', href: 'javascript:void(0);', target: '_self' }).append($('<i/>').attr({ class: 'fas fa-window-close' }));
