            ...
            if(\File::exists($file)) {
                ...
                if($this->option('seed') || $this->option('all')) {
                    ...
                    if($this->option('grant') || $this->option('all')) {
                        ...
                        $permissionIds = \EscaliersSolution\LaravelAdmin\Models\Permission::whereGroup($moduleName)->pluck('id');
                        \EscaliersSolution\LaravelAdmin\Models\Role::whereUnrestricted(0)->get()->each(fn($role) => $role->permissions()->attach($permissionIds));
                    }
                }
            }
            else {
