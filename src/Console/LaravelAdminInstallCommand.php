...
use ReflectionClass;
use LaravelAdmin;
use EscaliersSolution\LaravelAdmin\Database\Seeders\DatabaseSeeder;
...
class LaravelAdminInstallCommand extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            if(!$update || $force) $this->call('vendor:publish', ['--tag' => 'config', '--force' => true, '--provider' => 'EscaliersSolution\\LaravelAdmin\\LaravelAdminServiceProvider']);
            ...
            if($replaceConfig) $this->call('vendor:publish', ['--tag' => 'admin-config', '--force' => $replaceConfig, '--provider' => 'EscaliersSolution\\LaravelAdmin\\LaravelAdminServiceProvider']);
            
            // publish assets
            $this->info("Publishing assets...");
            $this->call('vendor:publish', ['--tag' => 'primary-assets', '--force' => $force || $update, '--provider' => 'EscaliersSolution\\LaravelAdmin\\LaravelAdminServiceProvider']);
            ...
            if($replaceAssets) $this->call('vendor:publish', ['--tag' => 'secondary-assets', '--force' => $replaceAssets, '--provider' => 'EscaliersSolution\\LaravelAdmin\\LaravelAdminServiceProvider']);
            ...
            if(!file_exists(resource_path('views/vendor/laravel-admin/partials/footer.blade.php')) && $this->confirm('Do you want to override the footer in your project?', true)) {
                ...
                $this->call('vendor:publish', ['--tag' => 'footer', '--force' => false, '--provider' => 'EscaliersSolution\\LaravelAdmin\\LaravelAdminServiceProvider']);
            }
            ...
            if($replaceSeeders) $this->call('vendor:publish', ['--tag' => 'seeders', '--force' => $replaceSeeders, '--provider' => 'EscaliersSolution\\LaravelAdmin\\LaravelAdminServiceProvider']);
            ...
            $this->call('vendor:publish', ['--tag' => 'data', '--force' => $replaceData, '--provider' => 'EscaliersSolution\\LaravelAdmin\\LaravelAdminServiceProvider']);
            ...
            if(!$update && $this->confirm('Do you want to overwrite DatabaseSeeder in your Project from LaravelAdmin? (Recommended if installing in a fresh project)', (config('app.env') != 'production'))) {
                $this->call('vendor:publish', ['--tag' => 'database-seeder', '--force' => true, '--provider' => 'EscaliersSolution\\LaravelAdmin\\LaravelAdminServiceProvider']);
            }
            ...
            if(!$empty) {
                ...
                $roleAdmin = \EscaliersSolution\LaravelAdmin\Models\Role::where('name', 'Admin')->first();
                if($roleAdmin && $roleAdmin->users()->doesntExist()) {
                    if($this->confirm('Do you want to create an Administrator user?', true)) {
                        $user = null;
                        do {
                            $user = new \EscaliersSolution\LaravelAdmin\Models\User;
                        ...
                        if($user) {
                            ...
                            $user->active = 1;
                            $validator = \Illuminate\Support\Facades\Validator::make($user->makeVisible('password')->toArray(), \EscaliersSolution\LaravelAdmin\Models\User::validationRules());
            ...
            if(!$update) {
                ...
                if (file_exists(app_path('User.php')) || file_exists(app_path('Models/User.php'))) {
                    ...
                    if ($modelContent !== false) {
                        ...
                        $modelContent = str_replace('extends Authenticatable', "extends \EscaliersSolution\LaravelAdmin\Models\User", $modelContent);
                    ...
                    $this->warn('You will need to update this manually.  Change "extends Authenticatable" to "extends \EscaliersSolution\LaravelAdmin\Models\User" in your User model and generate a policy for it with "php artisan make:xpolicy UserPolicy"');
                }
            }
